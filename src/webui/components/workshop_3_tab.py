...
def _get_config_value(
    ...
    tab_name = "workshop_3"  # Hardcode or derive if needed
...
async def _handle_new_step(
    ...
    if not hasattr(webui_manager, "wk3_chat_history"):
        logger.error(
            "Attribute 'wk3_chat_history' not found in webui_manager! Cannot add chat message."
        )
        # Initialize it maybe? Or raise an error? For now, log and potentially skip chat update.
        webui_manager.wk3_chat_history = []  # Initialize if missing (consider if this is the right place)
    ...
    webui_manager.wk3_chat_history.append(chat_message)
...
def _handle_done(webui_manager: WebuiManager, history: AgentHistoryList):
    ...
    webui_manager.wk3_chat_history.append(
...
async def _ask_assistant_callback(
    ...
    webui_manager.wk3_response_event = asyncio.Event()
    webui_manager.wk3_user_help_response = None  # Reset previous response

    try:
        logger.info("Waiting for user response event...")
        await asyncio.wait_for(
            webui_manager.wk3_response_event.wait(), timeout=3600.0
        )  # Long timeout
        ...
        webui_manager.wk3_response_event = None  # Clear the event
        return {"response": "Timeout: User did not respond."}  # Inform the agent

    response = webui_manager.wk3_user_help_response
    webui_manager.wk3_chat_history.append(
        {"role": "user", "content": response}
    )  # Show user response in chat
    webui_manager.wk3_response_event = (
...
async def run_agent_task(
    ...
    user_input_comp = webui_manager.get_component_by_id("workshop_3.user_input")
    run_button_comp = webui_manager.get_component_by_id("workshop_3.run_button")
    stop_button_comp = webui_manager.get_component_by_id(
        "workshop_3.stop_button"
    )
    pause_resume_button_comp = webui_manager.get_component_by_id(
        "workshop_3.pause_resume_button"
    )
    clear_button_comp = webui_manager.get_component_by_id(
        "workshop_3.clear_button"
    )
    chatbot_comp = webui_manager.get_component_by_id("workshop_3.chatbot")
    history_file_comp = webui_manager.get_component_by_id(
        "workshop_3.agent_history_file"
    )
    gif_comp = webui_manager.get_component_by_id("workshop_3.recording_gif")
    browser_view_comp = webui_manager.get_component_by_id(
        "workshop_3.browser_view"
    )
    ...
    webui_manager.wk3_chat_history.append({"role": "user", "content": task})
    ...
    yield {
        ...
        chatbot_comp: gr.update(value=webui_manager.wk3_chat_history),
    ...
    if not webui_manager.wk3_controller:
        webui_manager.wk3_controller = CustomController(
            ask_assistant_callback=ask_callback_wrapper
        )
        await webui_manager.wk3_controller.setup_mcp_client(mcp_server_config)
    ...
    try:
        ...
        if not keep_browser_open:
            if webui_manager.wk3_browser_context:
                logger.info("Closing previous browser context.")
                await webui_manager.wk3_browser_context.close()
                webui_manager.wk3_browser_context = None
            if webui_manager.wk3_browser:
                logger.info("Closing previous browser.")
                await webui_manager.wk3_browser.close()
                webui_manager.wk3_browser = None

        # Create Browser if needed
        if not webui_manager.wk3_browser:
            ...
            webui_manager.wk3_browser = CustomBrowser(
        ...
        if not webui_manager.wk3_browser_context:
            ...
            webui_manager.wk3_browser_context = (
                await webui_manager.wk3_browser.new_context(config=context_config)
            )

        # --- 5. Initialize or Update Agent ---
        webui_manager.wk3_agent_task_id = str(uuid.uuid4())  # New ID for this task run
        os.makedirs(
            os.path.join(save_agent_history_path, webui_manager.wk3_agent_task_id),
            exist_ok=True,
        )
        history_file = os.path.join(
            save_agent_history_path,
            webui_manager.wk3_agent_task_id,
            f"{webui_manager.wk3_agent_task_id}.json",
        )
        gif_path = os.path.join(
            save_agent_history_path,
            webui_manager.wk3_agent_task_id,
            f"{webui_manager.wk3_agent_task_id}.gif",
        )
        ...
        if not webui_manager.wk3_agent:
            logger.info(f"Initializing new agent for task: {task}")
            if not webui_manager.wk3_browser or not webui_manager.wk3_browser_context:
                raise ValueError(
                    "Browser or Context not initialized, cannot create agent."
                )
            webui_manager.wk3_agent = BrowserUseAgent(
                task=task,
                llm=main_llm,
                browser=webui_manager.wk3_browser,
                browser_context=webui_manager.wk3_browser_context,
                controller=webui_manager.wk3_controller,
            ...
            webui_manager.wk3_agent.state.agent_id = webui_manager.wk3_agent_task_id
            webui_manager.wk3_agent.settings.generate_gif = gif_path
        else:
            webui_manager.wk3_agent.state.agent_id = webui_manager.wk3_agent_task_id
            webui_manager.wk3_agent.add_new_task(task)
            webui_manager.wk3_agent.settings.generate_gif = gif_path
            webui_manager.wk3_agent.browser = webui_manager.wk3_browser
            webui_manager.wk3_agent.browser_context = webui_manager.wk3_browser_context
            webui_manager.wk3_agent.controller = webui_manager.wk3_controller

        # --- 6. Run Agent Task and Stream Updates ---
        agent_run_coro = webui_manager.wk3_agent.run(max_steps=max_steps)
        agent_task = asyncio.create_task(agent_run_coro)
        webui_manager.wk3_current_task = agent_task  # Store the task

        last_chat_len = len(webui_manager.wk3_chat_history)
        while not agent_task.done():
            is_paused = webui_manager.wk3_agent.state.paused
            is_stopped = webui_manager.wk3_agent.state.stopped
            ...
            update_dict = {}
            if webui_manager.wk3_response_event is not None:
                update_dict = {
                ...
                while (
                        webui_manager.wk3_response_event is not None
            ...
            if len(webui_manager.wk3_chat_history) > last_chat_len:
                update_dict[chatbot_comp] = gr.update(
                    value=webui_manager.wk3_chat_history
                )
                last_chat_len = len(webui_manager.wk3_chat_history)

            # Update Browser View
            if headless and webui_manager.wk3_browser_context:
                try:
                    screenshot_b64 = (
                        await webui_manager.wk3_browser_context.take_screenshot()
                    )
                    if screenshot_b64:
        ...
        # --- 7. Task Finalization ---
        webui_manager.wk3_agent.state.paused = False
        webui_manager.wk3_agent.state.stopped = False
        final_update = {}
        try:
            ...
            webui_manager.wk3_agent.save_history(history_file)
            ...
            if not any(
                    ...
                    for msg in webui_manager.wk3_chat_history
                    if msg.get("role") == "assistant"
            ):
                webui_manager.wk3_chat_history.append(
                    {"role": "assistant", "content": "**Task Cancelled**."}
                )
            final_update[chatbot_comp] = gr.update(value=webui_manager.wk3_chat_history)
        except Exception as e:
            ...
            webui_manager.wk3_current_task = None  # Clear the task reference
        ...
        webui_manager.wk3_current_task = None  # Ensure state is reset
        yield {
            user_input_comp: gr.update(
...
async def handle_submit(
    ...
    if webui_manager.wk3_response_event and not webui_manager.wk3_response_event.is_set():
        logger.info(f"User submitted assistance: {user_input_value}")
        webui_manager.wk3_user_help_response = (
            user_input_value if user_input_value else "User provided no text response."
        )
        webui_manager.wk3_response_event.set()
        ...
        yield {
            ...
            webui_manager.get_component_by_id(
                "workshop_3.run_button"
            ): gr.update(value="⏳ En cours...", interactive=False),
        }
    # Check if a task is currently running (using _current_task)
    elif webui_manager.wk3_current_task and not webui_manager.wk3_current_task.done():
        logger.warning(
...
async def handle_stop(webui_manager: WebuiManager):
    ...
    agent = webui_manager.wk3_agent
    task = webui_manager.wk3_current_task
...
async def handle_clear(webui_manager: WebuiManager):
    ...
    webui_manager.wk3_current_task = None

    if webui_manager.wk3_controller:
        await webui_manager.wk3_controller.close_mcp_client()
        webui_manager.wk3_controller = None
    webui_manager.wk3_agent = None

    # Reset state stored in manager
    webui_manager.wk3_chat_history = []
    webui_manager.wk3_response_event = None
    webui_manager.wk3_user_help_response = None
    webui_manager.wk3_agent_task_id = None

    logger.info("Agent state and browser resources cleared.")

    # Reset UI components
    return {
        webui_manager.get_component_by_id("workshop_3.chatbot"): gr.update(
            value=[]
        ),
        webui_manager.get_component_by_id("workshop_3.user_input"): gr.update(
            value="", placeholder="Entrer votre tâche ici..."
        ),
        webui_manager.get_component_by_id(
            "workshop_3.agent_history_file"
        ): gr.update(value=None),
        webui_manager.get_component_by_id("workshop_3.recording_gif"): gr.update(
            value=None
        ),
        webui_manager.get_component_by_id("workshop_3.browser_view"): gr.update(
            value="<div style='...'>Browser Cleared</div>"
        ),
        webui_manager.get_component_by_id("workshop_3.run_button"): gr.update(
            value="▶️ Envoyer", interactive=True
        ),
        webui_manager.get_component_by_id("workshop_3.stop_button"): gr.update(
            interactive=False
        ),
        webui_manager.get_component_by_id(
            "workshop_3.pause_resume_button"
        ): gr.update(value="⏸️ Pause", interactive=False),
        webui_manager.get_component_by_id("workshop_3.clear_button"): gr.update(
            interactive=True
        ),
    }
...
def create_workshop_3_tab(webui_manager: WebuiManager):
    """
    Create the run agent tab, defining UI, state, and handlers.
    """
    request=("1. Naviguer vers https://browser-use.github.io/stress-tests/challenge.html \n"
             "2. Effectuer les 31 tâches")

    webui_manager.init_wk3_agent()

    # --- Define UI Components ---
    tab_components = {}
    with gr.Column():
        chatbot = gr.Chatbot(
            lambda: webui_manager.wk3_chat_history,  # Load history dynamically
    ...
    webui_manager.add_components(
        "workshop_3", tab_components
    )  # Use "workshop_3" as tab_name prefix
